...
export function MentorOverview() {
  ...
  const mentor = state.mentor
      ...
      new Date(mentorshipSession.scheduledAt).toDateString() === new Date().toDateString()
  )[0]
          ...
          <CardContent>
            {upcomingMentorshipSession ? (
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <Avatar className="h-12 w-12">
                    ...
                    <AvatarFallback>
                      {state.mentees.find((m) => m.id === upcomingMentorshipSession.menteeId)?.name?.split(' ').map((n: string) => n[0]).join('') || 'M'}
                    </AvatarFallback>
                  </Avatar>
                  <div className="flex-1">
                    <h4 className="font-semibold">{upcomingMentorshipSession.topic}</h4>
                    <p className="text-sm text-muted-foreground">
                      with {state.mentees.find((m) => m.id === upcomingMentorshipSession.menteeId)?.name || 'Mentee'}
                    </p>
                  </div>
                  <Badge variant="outline" className="text-green-600 border-green-200">
                    {new Date(upcomingMentorshipSession.scheduledAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </Badge>
                </div>
                <div className="text-center py-2 px-4 bg-green-50 dark:bg-green-950/20 rounded-lg border border-green-200 dark:border-green-800">
                  <p className="text-sm text-green-700 dark:text-green-300 font-medium">Mentoring session scheduled</p>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground mb-4">No mentoring sessions today</p>
                <Button 
                  variant="outline" 
                  onClick={() => setShowScheduleModal(true)}
                  className="gap-2"
