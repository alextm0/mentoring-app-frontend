...
type AppAction =
  ...
  | { type: 'UPDATE_SESSION_STATUS'; sessionId: number; status: Session['status'] }
  ...
  | { type: 'UPDATE_MENTEE_PROGRESS'; menteeId: number; progress: number }
...
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case 'UPDATE_MENTEE_PROGRESS':
      return {
        ...state,
        mentees: state.mentees.map(mentee =>
          mentee.id === action.menteeId
            ? { ...mentee, progress: action.progress }
            : mentee
        )
      }
